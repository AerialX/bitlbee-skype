...
static char *prplcb_set_channel_names(set_t *set, char *value);
static void prplcb_channel_names_load(struct im_connection* ic);
...
static void purple_init( account_t *acc )
{
	...
	set_add(&acc->set, "channel_names", NULL, prplcb_set_channel_names, acc);
}
...
static GHashTable* globalChannelNames = NULL;

GHashTable* prplcb_lookup_names(struct im_connection* ic, bool create)
{
	if (!globalChannelNames)
		globalChannelNames = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);

	GHashTable* data = g_hash_table_lookup(globalChannelNames, ic->acc->tag);
	if (!data && create) {
		data = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, g_free);
		g_hash_table_insert(globalChannelNames, g_strdup(ic->acc->tag), data);
	}

	return data;
}

static const char* prplcb_lookup_name(struct im_connection* ic, const char* id)
{
	const char* value = g_hash_table_lookup(prplcb_lookup_names(ic, true), id) ?: id;
	if (value[0] == '#')
		return value + 1;
	return value;
}

static int prplcb_channel_names_skip = FALSE;
static void prplcb_channel_names(GHashTable* table, const char* channelNames)
{
...
static void prplcb_channel_names_save(struct im_connection* ic)
{
	char setting[0x1000] = "";
	int first = TRUE;
	GHashTableIter iter;
	gpointer key, value;
	g_hash_table_iter_init(&iter, prplcb_lookup_names(ic, true));
	...
	prplcb_channel_names_skip = TRUE;
	set_setstr(&ic->acc->set, "channel_names", setting);
	prplcb_channel_names_skip = FALSE;
}
static void prplcb_channel_names_load(struct im_connection* ic)
{
	prplcb_channel_names(prplcb_lookup_names(ic, true), set_getstr(&ic->acc->set, "channel_names"));
}
static char *prplcb_set_channel_names(set_t *set, char *value)
{
	if (!prplcb_channel_names_skip) {
		account_t *acc = set->data;
		if (acc && acc->ic)
			prplcb_channel_names(prplcb_lookup_names(acc->ic, true), value);
	}
	return value;
}
static int prplcb_irc_channel_rename(irc_channel_t* channel, const char* name)
{
	char reason[1024];
	...
	// Save name change for our rooms
	...
	if (channel->data && !g_strcmp0(type, "chat") && !g_strcmp0(chat_type, "groupchat")) {
		...
		if (ic->acc && ic->acc->prpl && !g_strcmp0(ic->acc->prpl->name, "skypeweb")) {
			GHashTable* table = prplcb_lookup_names(ic, false);
			if (table) {
				g_hash_table_insert(table, g_strdup(gc->title), g_strdup(name));
				prplcb_channel_names_save(ic);
			}
		}
	}
	return 1;
}
static void prplcb_cmd_renchan(irc_t* irc, char** cmd)
{
	irc_channel_t *ic;
	ic = irc_channel_by_name(irc, cmd[1]);
	if (ic) {
		if (prplcb_irc_channel_rename(ic, cmd[2]))
...
static void prplcb_cmd_close(irc_t* irc, char** cmd)
{
	irc_channel_t *ic;
	ic = irc_channel_by_name(irc, cmd[1]);
	if (ic) {
...
static void prplcb_cmd(irc_t* irc, char** cmd)
{
	...
	if (!strcmp(command, "rename")) {
		if (argCount != 2)
			...
			prplcb_cmd_renchan(irc, cmd);
	} else if (!strcmp(command, "close")) {
		if (argCount != 1)
			irc_rootmsg(irc, "Expected 1 argument");
		else
			prplcb_cmd_close(irc, cmd);
	} else
		irc_rootmsg(irc, "Unknown prpl command. Expected one of: rename, close");
}
...
void prplcb_conv_new( PurpleConversation *conv )
{
	if( conv->type == PURPLE_CONV_TYPE_CHAT )
	{
		...
		imcb_chat_name_hint(gc, prplcb_lookup_name(ic, conv->name));
		const char* topic = purple_conv_chat_get_topic(PURPLE_CONV_CHAT(conv));
		if (topic) {
			char* value = g_strdup(topic);
			imcb_chat_topic(gc, NULL, value, 0);
			g_free(value);
		}
		conv->ui_data = gc;
		gc->data = conv;
...
void purple_initmodule()
{
	...
	root_command_add("prpl", 1, prplcb_cmd, 0);
}
